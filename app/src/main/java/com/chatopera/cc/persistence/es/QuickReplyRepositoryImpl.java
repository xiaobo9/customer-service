...
public class QuickReplyRepositoryImpl implements QuickReplyEsCommonRepository {
    ...
    public Page<QuickReply> getByCateAndUser(String cate, String q, String user, final int p, final int ps) {
        ...
        builder.must(termQuery("cate", cate));

        if (!StringUtils.isBlank(q)) {
            builder.must(new QueryStringQueryBuilder(q).defaultOperator(Operator.AND));
        }

        NativeSearchQueryBuilder searchQueryBuilder = new NativeSearchQueryBuilder().withQuery(builder).withQuery(termQuery("creater", user)).withSort(new FieldSortBuilder("top").unmappedType("boolean").order(SortOrder.DESC)).withSort(new FieldSortBuilder("updatetime").unmappedType("date").order(SortOrder.DESC));
...
//        QueryBuilder endFilter = QueryBuilders.boolQuery().should(QueryBuilders.missingQuery("endtime")).should(QueryBuilders.rangeQuery("endtime").to(new Date().getTime()));
        QueryBuilder beginFilter = QueryBuilders.boolQuery().should(QueryBuilders.rangeQuery("begintime").from(new Date().getTime()));
        QueryBuilder endFilter = QueryBuilders.boolQuery().should(QueryBuilders.rangeQuery("endtime").to(new Date().getTime()));
    ...
    public Page<QuickReply> getByOrgiAndType(String orgi, String type, String q, Pageable page) {
        ...
        BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery();
        boolQueryBuilder.must(termQuery("orgi", orgi));
        mustTermQuery(boolQueryBuilder, "type", type);
    ...
    public List<QuickReply> getQuickReplyByOrgi(String orgi, String cate, String type, String q) {
        ...
        mustTermQuery(boolQueryBuilder, "cate", cate);
        mustTermQuery(boolQueryBuilder, "type", type);
    ...
    public void mustTermQuery(BoolQueryBuilder builder, String name, String value) {
        if (StringUtils.isNotBlank(value)) {
            builder.must(termQuery(name, value));
        }
    }
