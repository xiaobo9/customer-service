...
import com.github.xiaobo9.helper.IACDPolicyService;
...
public class AgentStatus implements java.io.Serializable, Comparable<AgentStatus> {
    ...
    private static IACDPolicyService policyService;
    /**
     *
     */
    ...
    private String status = AgentStatusEnum.NOTREADY.toString();        //坐席状态
    ...
    public int getMaxusers() {
        return skills.keySet().stream()
                .map(key -> policyService.initSessionConfig(key, orgi))
                .filter(Objects::nonNull)
                .mapToInt(SessionConfig::getMaxuser)
                .sum();
    }
    ...
    public int getInitmaxusers() {
        int allInitmaxuser = this.skills.keySet().stream().mapToInt(k -> {
            SessionConfig sessionConfig = policyService.initSessionConfig(k, this.orgi);
...
//        }
    ...
    public static void policyService(IACDPolicyService policyService) {
        AgentStatus.policyService = policyService;
    }
}
